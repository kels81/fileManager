...
//    }
    ...
    private void updateContent() {
        Usuarios user = (Usuarios) VaadinSession.getCurrent().getAttribute(
                Usuarios.class.getName());
        if (user != null && "Director".equals(user.getuRole())) {
            // Authenticated user
            setContent(new MainView());
            removeStyleName("loginview");
            getNavigator().navigateTo(getNavigator().getState());
    ...
    @Subscribe
    public void userLoginRequested(final UserLoginRequestedEvent event) {
        Usuarios user = getDataProvider().authenticate(event.getUserName(),
                event.getPassword());
        VaadinSession.getCurrent().setAttribute(Usuarios.class.getName(), user);
        updateContent();
    }
